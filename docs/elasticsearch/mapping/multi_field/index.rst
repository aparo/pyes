...
The **multi_field** type allows to map several :doc:`core_types <./../core_types/index>` of the same value. This can come very handy, for example, when wanting to map a **string** type, once when its **analyzed** and once when its **not_analyzed**. For example:
