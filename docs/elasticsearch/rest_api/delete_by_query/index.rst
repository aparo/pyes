...
The delete by query API allows to delete documents from one or more indices and one or more types based on a query. The query can either be provided using a simple query string as a parameter, or using the :doc:`Query DSL <./../query_dsl/index>` defined within the request body. Here is an example:
...
The delete by query can use the :doc:`Query DSL <./../query_dsl/index>` within its body in order to express the query that should be executed and delete all documents. The body content can also be passed as a REST parameter named **source**.
